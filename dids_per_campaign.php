...
$sql = "
    SELECT
        ...
        COUNT(*) AS call_count,
        CEIL(COUNT(*) / ($days * 50)) AS dids_needed
    FROM
        vicidial_log
    WHERE
...
if ($result->num_rows > 0) {
    ...
    fputcsv($csv_file, ['Campaign ID', 'Area Code', 'Call Count', 'DIDs Needed']);
    ...
    while ($row = $result->fetch_assoc()) {
        ...
        // Write to CSV file
        fputcsv($csv_file, [$row["campaign_id"], $row["area_code"], $row["call_count"], $row["dids_needed"]]);
    }
